...
function saveProductSilent() {
    ...
    if (!product || !piece || !quantity || !shelfLife) {
        console.warn('⚠️ Nisu popunjena sva polja za glasovni unos');
        return;
    }
...
// ===== ČIŠĆENJE POLJA ZA GLASOVNI UNOS =====
function ocistiPolja() {
    ...
    if (typeof window.resetCurrentProductData === 'function') {
...
function updateHeaderLanguage() {
    ...
    if (backText) backText.textContent = ht('nazad');
    if (invText) invText.textContent = ht('stanje');
    if (shopText) shopText.textContent = ht('spisak');
    if (exitText) exitText.textContent = ht('odustani');
    ...
    if (headerTitle) {
        ...
        headerTitle.textContent = screenTitles[lang]?.[currentScreenName] || screenTitles['en'][currentScreenName] || '📂 Kategorije';
    }
...
let isProcessingVoice = false; // Blokada za petlju

function processVoiceCommand(command) {
    if (!command || isProcessingVoice) return; // Ako već obrađuje, prekini!

    console.log('🎤 processVoiceCommand prima:', command);
    
    // Ako stigne čisto "add", a već smo u procesu, ne šalji dalje
    if (command.toString().trim().toLowerCase() === 'add') {
        console.warn('⚠️ Blokiran ponovljeni "add" poziv.');
        return;
    }

    isProcessingVoice = true;

    const cmd = command.toString().toLowerCase().trim();
    ...
    // Ako ništa gore nije presrelo, tek onda prosledi na detaljniju obradu u voiceCommands.js
    if (typeof window.voiceCommand === 'function') {
        console.log('📞 Pozivam window.voiceCommand');
        window.voiceCommand(command);
    } else {
